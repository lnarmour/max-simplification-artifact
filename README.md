...
This repository contains code samples that complement the POPL 2025 conference paper titled, "Maximal Simplification of Polyhedral Reductions". The paper proposes extensions to the simplification algorithm, which obtains asmpytotic complexity improvments automatically. The included (A)lpha to (C) (C)ompiler, `acc`, script contains a partial proof-of-concept implementation of the splitting methods proposed in the paper. 
...
The `inputs` directory contains several polynomial complexity input programs. This directory contains the following three scripts:
1. `acc`: takes an input Alpha program and generates simplified C implementations with improved asymptotic complexity.
1. `build.sh`: runs `acc` on the input programs, generates C code for input program and each of the discovered simplifications, and runs correctness checks.
...
All of the input programs, except `inputs/example_4d.alpha`, admit a linear complexity, $O(N)$, representation discoverable by simplification, some of which require splitting as discussed in the paper. 

The 4-dimensional one, however, indeed has a $O(N^2)$ simplification but the the current partial implementation does not autonomously handle it. We provide a manual simplification of this which has the expected complexity using the techniques described in the paper.
...
```
This creates subdirectory called `prefix_sum` which can be compiled with the following command:
```
make -C prefix_sum
```
...
```
(Note the `$` are part of the shell prompt in this instance and not the command)
...
Given a convex domain expressed as an [isl](https://libisl.sourceforge.io/manual.pdf) basic set, the `-l` and `--face-lattice` options can be used to build the corresponding face lattice. 
...
# The single-step (Section 3.4) involves selecting a reuse vector (rho) in R
# and applying Theorem 5 from GR06.
# 
